...
function PrintTest() {
    ...
    $.post("/Invoice/Invoice/PrintInvoice?InvoiceNo=" + InvoiceNo, InvoiceNo);
...
function RegionAndDistrict() {
	...
	objBO.to = $('#txtTo').val();
	objBO.login_id = Active.userId;
	objBO.Logic = "DashBoard:Admin-RegionAndDistrict-New";
	$.ajax({
		method: "POST",
		url: url,
		data: JSON.stringify(objBO),
			...
			if (data.ResultSet.Table.length > 0) {
				...
				$.each(data.ResultSet.Table, function (key, val) {
					...
					if (val.regionName == 'Kumaon') {
                        ...
                        $('#txtKumaonActiveTotal').text(val.tActiveCenter + '/' + val.tTotalCenter);
					...
					if (val.regionName == 'Garhwal') {
                        ...
                        $('#txtGarwalActiveTotal').text(val.tActiveCenter + '/' + val.tTotalCenter);
...
function CentreByDistrict(district) {
	...
	objBO.to = $('#txtTo').val();
	objBO.login_id = Active.userId;
	objBO.Logic = "DashBoard:Admin-CentreByDistrict-New";
	$.ajax({
		method: "POST",
		url: url,
		data: JSON.stringify(objBO),
