...
function BookingListForApproval() {
	...
	objBO.from = Properdate($('#txtFrom').val(), '-');
	objBO.to = Properdate($('#txtTo').val(), '-');
	objBO.login_id = Active.userId;
	objBO.Logic = "BookingListForApproval";
	if (objBO.Prm1 == 'Approved')
			...
			if (data.ResultSet.Table.length > 0) {
				...
				$.each(data.ResultSet.Table, function (key, val) {
					...
					if (val.ApproveDate.length > 4) {
						...
						tbody += "<tr>";
						tbody += "<td><button id='btnApprove' class='btn-success btn-go'>Approve</button></td>";
					}
			...
			if (data.ResultSet.Table.length > 0) {
				...
				$.each(data.ResultSet.Table1, function (key, val) {
					...
					if (val.IsCancelled=='1')
						tbody += "<tr style='background:#ffbaba'>";
					else
						tbody += "<tr>";
...
function ApproveAll() {
	if (confirm('Are you sure to Approve All?')) {
		var url = config.baseUrl + "/api/Report/MIS_InsertUpdateReports";
		var objBO = {};
		var visitNo = [];
		$('#tblPatientRegister tbody tr').each(function () {
			visitNo.push($(this).find('td:eq(1)').text());
		});
		objBO.VisitNo = '';
		objBO.MobileNo = '-';
		objBO.remark = '-';
		objBO.Prm1 = visitNo.join('|');
		objBO.login_id = Active.userId;
		objBO.Logic = "ApproveAllReport";
		$.ajax({
			method: "POST",
			url: url,
			data: JSON.stringify(objBO),
...
function TotalRecord() {
	var total = 'Total Record : ' + $('#tblPatientRegister tbody tr').length;
	$('#txtTotalRecord').text(total);
}
function TotalAmount() {
	...
	var amount = 'Total Amount : ' + total.toFixed(2);
